...
class AttendanceController extends Controller
{
        ...
        if ($selectedClass) {
            ...
            foreach ($slots as $slot) {
                ...
                for ($offset = 0; $offset < $count; $offset++) {
                    ...
                    $periods->push(['key' => "slot-{$slot->id}-{$index}", 'slot_id' => $slot->id, 'index' => $index,
                        'label' => 'Période '.$index, 'start' => $slot->start_time?->copy()->addHours($offset)->format('H:i'),
                        'end' => $slot->start_time?->copy()->addHours($offset + 1)->format('H:i'),
                        'subject' => $slot->classSubject?->subject?->name_fr ?? 'Cours non renseigné', 'class_subject_id' => $slot->class_subject_id]);
                }
            }
            $enrollments = StudentEnrollment::where(['class_group_id' => $selectedClass->id, 'academic_year_id' => $activeYear->id, 'status' => StudentEnrollment::STATUS_ACTIVE])
                ->with('student')->get()->sortBy('student.last_name')->values();
            if ($periods->isNotEmpty() && $enrollments->isNotEmpty()) {
                Absence::whereDate('absence_date', $date)->whereIn('student_enrollment_id', $enrollments->pluck('id'))
                    ->whereIn('timetable_slot_id', $periods->pluck('slot_id')->unique())->get()
    ...
    public function store(Request $request)
    {
        ...
        abort_if($definitions->isEmpty(), 422, 'Aucune période valide pour cette classe et cette date.');
        $enrollments = StudentEnrollment::where(['class_group_id' => $class->id, 'academic_year_id' => $year?->id, 'status' => StudentEnrollment::STATUS_ACTIVE])->pluck('id');
        $attendance = $request->input('attendance', []); $saved = 0;
        foreach ($enrollments as $enrollmentId) foreach ($definitions as $period) {
            $query = Absence::where(['student_enrollment_id' => $enrollmentId, 'timetable_slot_id' => $period['slot_id'], 'timetable_period_index' => $period['index']])->whereDate('absence_date', $date);
            if (!empty($attendance[$enrollmentId][$period['key']])) {
                Absence::updateOrCreate(['student_enrollment_id' => $enrollmentId, 'absence_date' => $date->toDateString(), 'timetable_slot_id' => $period['slot_id'], 'timetable_period_index' => $period['index']], ['period' => $period['label'], 'class_subject_id' => $period['class_subject_id'], 'hours' => 1, 'is_justified' => false, 'recorded_by' => Auth::id()]);
                $saved++;
