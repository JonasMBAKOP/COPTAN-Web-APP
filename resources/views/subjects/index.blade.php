...
<div x-data="{
    tab: '{{ request('tab', 'all') }}',
    fabOpen: false,
    categoryModal: {{ $errors->has('name_fr') || $errors->has('name_en') ? 'true' : 'false' }},
    categoryListModal: {{ request()->boolean('categories') ? 'true' : 'false' }},
    editCategory: null,
    categories: @js($categories->map(fn ($category) => ['id' => $category->id, 'name_fr' => $category->name_fr, 'name_en' => $category->name_en, 'subjects_count' => $category->subjects_count])->values()),
            ...
            <div class="max-h-[60vh] divide-y divide-gray-100 overflow-y-auto">
                ...
                <template x-for="category in categories" :key="category.id">
                    <div class="flex items-center gap-4 px-6 py-4">
                        <div class="min-w-0 flex-1"><p class="truncate text-sm font-bold text-gray-800" x-text="category.name_fr"></p><p x-show="category.name_en" class="mt-0.5 truncate text-xs italic text-gray-400" x-text="category.name_en"></p></div>
...
@push('fixed_footer')
<div class="hidden fixed bottom-0 left-0 md:left-64 right-0 z-20 md:flex
            bg-white border-t border-gray-200 shadow-md
            px-5 py-2.5 flex-col sm:flex-row sm:items-center
            justify-between gap-2 text-xs text-gray-500">
