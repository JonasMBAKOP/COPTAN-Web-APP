                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <select name="subject_category_id"
                            ...
                            <option value="{{ $cat->id }}"
                                    {{ old('subject_category_id') == $cat->id ? 'selected' : '' }}>
                                {{ $cat->name_fr }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="rounded-3xl border border-gray-100 bg-slate-50 p-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            ...
                            <label class="flex items-center gap-3 p-3 rounded-2xl border transition-colors cursor-pointer text-sm
                                          {{ old('type') === $t['val'] ? 'border-indigo-300 bg-indigo-50' : 'border-gray-200 bg-white hover:border-indigo-200' }}">
                                <input type="radio" name="type"
                                       value="{{ $t['val'] }}"
                                       {{ old('type') === $t['val'] ? 'checked' : '' }}
