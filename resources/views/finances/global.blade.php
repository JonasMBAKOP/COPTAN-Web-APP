...
.finance-track .finance-progress { min-width:0; }
...
@php
    ...
    $collectionRate = $expected > 0
        ? min(100, max(0, (int) round(($collected / $expected) * 100)))
        : 0;
...
<div class="finance-shell space-y-6">
        ...
        <div class="finance-soft-card finance-kpi finance-card rounded-2xl p-5 ring-1 ring-transparent">
            <div class="flex items-center justify-between gap-4">
                <div>
                    ...
                    <p class="mt-2 text-2xl font-black text-[#9A741B]">{{ $paidRate }}%</p>
                ...
                <div class="relative h-16 w-16">
                    ...
                    <span class="absolute inset-0 flex items-center justify-center text-xs font-black text-[#9A741B]">{{ $paidRate }}%</span>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="grid grid-cols-1 gap-6 xl:grid-cols-3">
        <div class="space-y-6 xl:col-span-2">
            <div class="finance-soft-card finance-card rounded-2xl p-5 lg:p-6">
                <div class="mb-5 flex flex-col gap-3 border-b border-[#E5EDF5] pb-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h3 class="text-base font-black text-[#163B5C]">Collecte mensuelle</h3>
            ...
            <div class="finance-soft-card finance-card rounded-2xl p-5 lg:p-6">
                <div class="mb-5 flex items-center justify-between border-b border-[#E5EDF5] pb-4">
                    <div>
                        <h3 class="text-base font-black text-[#163B5C]">Collecte par section</h3>
                    ...
                    @forelse($sectionStats as $secData)
                        @php
                            $rate = $secData->expected > 0
                                ...
                                : 0;
                        @endphp
                        <div class="rounded-2xl border border-[#E5EDF5] bg-white p-4">
                            <div class="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    <p class="font-black text-[#163B5C]">{{ $secData->section->name }}</p>
        ...
        <aside class="space-y-6">
            <div class="finance-soft-card finance-card rounded-2xl p-5">
                <h3 class="border-b border-[#E5EDF5] pb-4 text-base font-black text-[#163B5C]">Tranches de paiement</h3>
            ...
            <div class="finance-soft-card finance-card rounded-2xl p-5">
                <div class="flex items-center justify-between border-b border-[#E5EDF5] pb-4">
                    <h3 class="text-base font-black text-[#163B5C]">Paiements récents</h3>
                    <a href="{{ route('finances.payments') }}" class="text-xs font-black text-[#1F4E79] hover:underline">Tout voir</a>
                </div>
                <div class="mt-4 space-y-3">
                    @forelse($recentPayments as $p)
                        <div class="rounded-2xl border border-[#EDF3F8] bg-[#F8FBFE] p-3">
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    <p class="truncate text-xs font-black text-slate-800">{{ $p->studentEnrollment->student->full_name }}</p>
                                    <p class="mt-0.5 truncate text-[11px] font-semibold text-slate-400">{{ $p->studentEnrollment->classGroup->full_name }} · {{ $p->feeInstallment?->label }}</p>
                                </div>
                                <div class="text-right">
                                    <p class="whitespace-nowrap text-xs font-black text-[#2F7D57]">+{{ number_format($p->amount_paid) }}</p>
    ...
    @if($debtors->isNotEmpty())
        <section class="finance-soft-card finance-card overflow-hidden rounded-2xl">
            <div class="flex flex-col gap-2 border-b border-[#E5EDF5] bg-[#F8FBFE] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h3 class="text-base font-black text-[#163B5C]">Élèves avec impayés</h3>
                    <p class="text-xs font-semibold text-slate-400">Top 10 des soldes restants les plus élevés</p>
                </div>
                <span class="rounded-full bg-rose-50 px-3 py-1 text-xs font-black text-[#B45353]">{{ $debtors->count() }} débiteur(s)</span>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full min-w-[860px] text-left">
                    <thead class="bg-white text-[11px] font-black uppercase tracking-wide text-slate-400">
                        <tr class="border-b border-[#E5EDF5]">
                    ...
                    <tbody class="divide-y divide-[#EDF3F8] text-xs font-semibold text-slate-700">
                        @foreach($debtors->take(10) as $d)
                            <tr class="transition hover:bg-[#F8FBFE]">
                                <td class="px-5 py-4">
                                    <p class="font-black text-slate-800">{{ $d['enrollment']->student->full_name }}</p>
                                ...
                                <td class="px-4 py-4 text-right font-black text-[#2F7D57]">{{ number_format($d['paid']) }} FCFA</td>
                                <td class="px-4 py-4 text-right font-black text-[#B45353]">{{ number_format($d['remaining']) }} FCFA</td>
                                <td class="px-5 py-4 text-center no-print">
                                    <a href="{{ route('finances.student.receipt', $d['enrollment']) }}" class="inline-flex items-center justify-center rounded-xl border border-[#DCE8F3] bg-white px-3 py-2 text-[11px] font-black text-[#1F4E79] transition hover:bg-[#EEF6FC]">Voir dossier</a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
